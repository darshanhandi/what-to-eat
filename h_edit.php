...
<div class="header-nav">
      ...
      <a href="a_hospital.php">Back </a> </button> </div>
                  </div>
                  </nav>
...
if ($cid==''&& $cname=='' && $cmobile=='')
  ...
  $sql="select * from hospital where id='$cid'" ;
 ...
 <td>".$row["mobile"]."</td>
  <td>".$row["email"]."</td>
  <td>".$row["password"]."</td>
  <td> <button type='submit' class='btn btn-default' style='box-shadow:0px 0px 1px 1px blue'> <a href='h_update.php?id=$id'>Edit</a></button>&nbsp;
  <button type='submit' class='btn btn-default' style='box-shadow:0px 0px 1px 1px red'>
  <a href='h_delete.php?id=$id' onclick='return checkdelete()'>Delete</a> </button></td>";
}
echo "</table>";
  ...
  elseif(!$cname==''){
  $sql="select * from hospital where name='$cname'" ;
  $result=$conn->query($sql);
...
echo "</table>";
  ...
  elseif(!$cmobile==''){
  $sql="select * from hospital where mobile='$cmobile'" ;
  $result=$conn->query($sql);
  if($result-> num_rows > 0){
      while($row=$result->fetch_array()){
 $id= $row["id"];
